...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // The system back gesture navigates folder history first (Nova sets
        // window.__novaBack); only when there is nothing to go back to does it
        // fall through to the default behaviour and leave the app.
        getOnBackPressedDispatcher().addCallback(this, new OnBackPressedCallback(true) {
            @Override
            public void handleOnBackPressed() {
                webView.evaluateJavascript("(window.__novaBack && window.__novaBack()) ? 1 : 0", value -> {
                    if (!"1".equals(value)) {
                        setEnabled(false);
